...
class BossBarFolder: ArcartXConfigFolder<BossBar>(bukkitPlugin, "boss_bar", ::BossBar) {
    ...
    override fun onCreateFolder(plugin: JavaPlugin, folderPath: String) {
        this.createConfig(plugin,   "${folderPath}boss血条示例.yml")
    }

    override fun onSetFileID(config: BossBar, id: String) {
        config.id = id
    }
    ...
    override fun reload() {
        super.reload()
        bukkitPlugin.sendMessage(L(AXLanguageKey.LOAD_BOSSBAR, configs.size.toString()))
    }
}
